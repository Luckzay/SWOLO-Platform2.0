...
from netsv8.yolov8 import YoloBody
from utilsv8.utils import (cvtColor, get_classes, preprocess_input,
                         resize_image, show_config)
from utilsv8.utils_bbox import DecodeBox
...
class YOLO(object):
    ...
    def generate(self, onnx=False):
        ...
        # 尝试加载模型权重，如果键名不匹配则使用strict=False
        try:
            self.net.load_state_dict(torch.load(self.model_path, map_location=device))
        except RuntimeError as e:
            print(f"标准加载失败，尝试使用strict=False: {e}")
            # 尝试使用strict=False加载，只加载匹配的键
    ...
    #   检测图片
    #---------------------------------------------------#
    def detect_image(self, image, crop = False, count = False, return_detections=False):
        #---------------------------------------------------#
        #   计算输入图片的高和宽
        #---------------------------------------------------#
        ...
        for i, c in list(enumerate(top_label)):
            ...
            # 收集检测结果 [类别, 置信度, x, y, width, height]
            detections.append((predicted_class, score, left, top, right - left, bottom - top))

            label = '{} {:.2f}'.format(predicted_class, score)
            draw = ImageDraw.Draw(image)
            label_size = draw.textbbox((0,0), text=label,font=font)
            label = label.encode('utf-8')
            print(predicted_class, score, left, top, right - left, bottom - top)
